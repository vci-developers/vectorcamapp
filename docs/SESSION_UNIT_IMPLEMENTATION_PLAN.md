...
**Slice 2 — Reactive duplicate-identity banner (✅ Shipped).** Closes Deferred-to-3d #2 (and discovered Deferred-to-3d #8 was already closed by an earlier slice's cleanup).

Before: `state.collectionBatchFormErrors.duplicateIdentity` only updated when the user tapped "Continue to Imaging" — `SubmitSessionUnitForm` re-fetched `getSessionUnitScopedFormAnswersBySessionId` and ran `validateCollectionBatchIdentity` inline, writing the result back into `_state`. While the user was typing into an identity field, the banner stayed stale (and silent) until submit. After: typing into any identity field updates the banner immediately — derivation happens inside the VM's outer state pipeline, fed by a Room Flow that emits any time `form_answer` rows in the session change.

**Files modified:**

- `core/data/room/dao/FormAnswerDao.kt` — added `observeSessionUnitScopedFormAnswersBySessionId(sessionId: UUID): Flow<List<FormAnswerEntity>>`. Same `WHERE sessionId = :sessionId AND sessionUnitId IS NOT NULL` query; only the return shape changes. The pre-existing suspend `getSessionUnitScopedFormAnswersBySessionId` sibling was **deleted in the same diff** because both its call sites (the form VM in this slice + the list VM also in this slice) were converted to the new Flow — see "Why also flip the list VM" below.
- `core/domain/repository/FormAnswerRepository.kt` — added `fun observeSessionUnitScopedFormAnswersBySessionId(sessionId: UUID): Flow<Map<UUID, Map<Int, FormAnswer>>>`, deleted the suspend `getSessionUnitScopedFormAnswersBySessionId` declaration.
- `core/data/repository/FormAnswerRepositoryImplementation.kt` — added the `observe…` impl, deleted the suspend `getSessionUnitScopedFormAnswersBySessionId` impl. Net file change is +12/−7 lines.
- `collection_batch/form/presentation/CollectionBatchFormViewModel.kt` — added `private val _existingAnswersBySessionUnitId = formAnswerRepository.observeSessionUnitScopedFormAnswersBySessionId(sessionId)` (the canonical "second Flow" shape — exactly mirrors `CollectionBatchListViewModel._sessionUnits` and `IntakeViewModel._allCollectors`). Flipped the outer `state` chain from `_state.onStart { … }.stateIn(…)` to `combine(_state, _existingAnswersBySessionUnitId) { state, existingAnswers → … state.copy(collectionBatchFormErrors = …copy(duplicateIdentity = validatorOutput)) }.onStart { … }.stateIn(…)`. `SubmitSessionUnitForm` simplified: dropped the inline `getSessionUnitScopedFormAnswersBySessionId` re-fetch, dropped the inline `validateCollectionBatchIdentity` call, dropped the imperative `duplicateIdentity = identityResult.errorOrNull()` write into `_state`; now gates on `state.value.collectionBatchFormErrors.duplicateIdentity != null` against the public outer state — i.e. the same value the UI is reading. Per-field `validateFormAnswers` is untouched (Deferred-to-3d #5 deliberately defers that).
- `collection_batch/list/presentation/CollectionBatchListViewModel.kt` — added `private val _existingAnswersBySessionUnitId = formAnswerRepository.observeSessionUnitScopedFormAnswersBySessionId(sessionId)`. The outer `combine(_sessionUnits, _state) { … }` became `combine(_sessionUnits, _existingAnswersBySessionUnitId, _state) { … }`; the lambda's inline suspend `formAnswerRepository.getSessionUnitScopedFormAnswersBySessionId(sessionId)` call was replaced by reading the combine's `existingAnswersBySessionUnitId` parameter. `loadSessionUnitFormQuestions()` is still called inside the lambda — it remains a suspend read since `form_question` is essentially immutable during the screen's lifetime and Room's first-level cache makes the re-call free.
...
**Why `duplicateIdentity` is still a field on `CollectionBatchFormErrors`** (and not promoted to a top-level field on `CollectionBatchFormState` or exposed as a sibling StateFlow). It's purely derived now, so the field's stored value on `_state` is always whatever the initial value was (`null`) — the *effective* value the UI sees is the one stamped on by the outer combine. Considered pulling it off `CollectionBatchFormErrors` for honesty, but it would create an asymmetry with `formAnswerErrors` (which slice 2 leaves imperative per Deferred-to-3d #5) and would require touching the screen's read site. The screen already reads `state.collectionBatchFormErrors.duplicateIdentity` — keeping the same field as the read target is the smaller, screen-invariant change.

**Why no `distinctUntilChanged` on the `_state` projection.** First-pass design considered `_state.map { it.formQuestions }.distinctUntilChanged()` + `_state.map { it.formAnswersByQuestionId }.distinctUntilChanged()` as two separate combine inputs. The simpler "feed `_state` itself into the combine and read `state.formQuestions` / `state.formAnswersByQuestionId` inside the lambda" matches every other VM in the codebase and only fires the validator once per `_state` emission (which already covers both inputs). Validator is pure and cheap (a few `.trim()` + equality checks per identity question); re-running on the rare non-identity `_state` change is free.

**`existingAnswersBySessionUnitId` variable name.** Matches the parameter name on `ValidateCollectionBatchIdentityUseCase.invoke(...)` for read symmetry. The audit pass's §2 (variable-name consistency) will reconcile this against the `answersBySessionUnitId` name used in `CollectionBatchListViewModel` and pick one feature-wide convention; not slice 2's job.

**Self-edit case verified.** Hydrating an existing batch in edit mode does not false-flag: the use case's `existingAnswersBySessionUnitId.filterKeys { it != editingSessionUnitId }` already excludes the unit being edited, and `editingSessionUnitId = sessionUnitId` is passed through from the route field. Identical behavior to PR 3c's submit-time check, just running continuously instead of on-tap.

**Plan-compliance checklist.**

- ✅ **Rule 1** (add methods only when the call site lands) — DAO + repo additions are exactly the call site introduced in this diff.
- ✅ **Rule 2** (modify-don't-proliferate evaluated) — widening the existing `suspend get…` to return a Flow was considered and rejected per the explicit return-shape carve-out; sibling `observe…` is the correct shape and matches existing codebase precedent (`get*` ↔ `observe*` pairs on every repo).
- ✅ **Rule 4** (match conventions) — outer-combine derivation shape mirrors `IntakeViewModel:90`, `CollectionBatchListViewModel:55`, `ImagingViewModel:107`, `LandingViewModel:41`, etc.; `observe*` repo shape mirrors `SessionUnitRepository.observeSessionUnitsForSession` line-for-line.
- ✅ **Convention #4 / #18** — reactive derivation via `combine` in the VM, but specifically via the codebase's outer-combine idiom rather than a side `.launchIn` loop.
- ✅ **Convention #6 / #11** — repository owns the typed read; validation use case is unchanged; no validator logic leaked into `FormAnswerRepository`.
- ✅ **Convention #12** — identity validation remains per-questionId equality; only the trigger moved from imperative to reactive.

**Deferred-to-3d items closed by this slice.**

- ✅ **#2** (reactive banner). Shipped per the design above.
- ✅ **#8** (banner/toast string consolidation). Already done before slice 2 started — see updated strike-through in the Deferred list above.

**Deviations from plan worth recording.**

1. **Source of the cross-unit snapshot.** Plan §306 prescribed a `MutableStateFlow<Map<UUID, Map<Int, FormAnswer>>>` field on the VM, seeded from the existing `suspend get…` read inside `loadFormDetails`. Final implementation introduces an `observe*` sibling on `FormAnswerDao` + `FormAnswerRepository` and treats it as a Room Flow source — zero hand-rolled MSF on the VM. Rationale captured under "Why an `observe*` sibling…" above.
2. **Orchestration shape.** Plan §306 prescribed `combine(...).launchIn(viewModelScope)` writing back into `_state` via `.onEach`. Final implementation folds the cross-unit Flow into the outer `combine(_state, _existingAnswersBySessionUnitId) { … state.copy(…) }` that already builds the public `StateFlow`. Rationale captured under "Why derive in the outer combine…" above.

**Why also flip the list VM (and delete the suspend `get…` in the same diff).** After converting `SubmitSessionUnitForm`, the only remaining caller of `formAnswerRepository.getSessionUnitScopedFormAnswersBySessionId` was `CollectionBatchListViewModel:57-58`, where the suspend call already lived *inside* an existing `combine(_sessionUnits, _state) { … }` lambda — exactly the shape that asks to be a third combine input. Three reasons to bundle the list-VM flip + suspend deletion into slice 2 rather than deferring to audit-pass §1/§4/§5:

1. **Pattern parity.** Both consumers of cross-unit answers now use the identical `observe*` → combine-input shape. Splitting it across two PRs would leave the list VM on the suspend `get*` while the form VM was on `observe*`, a momentary inconsistency for no benefit.
2. **The suspend `get*` is genuinely dead after this slice.** Carrying a no-caller method through to PR 3's ship gate is exactly what the audit pass exists to remove — doing it in-context here costs three line-deletions across DAO + repo interface + impl.
3. **Bucket-name reactivity comes along for free.** With `observe*` feeding the list VM, any future `form_answer` mutation (e.g. editing a batch and returning to the list) re-derives bucket names without waiting for `_sessionUnits` to re-emit.

**Inherited debt / outstanding items worth logging (not fixed in this slice).**

1. **`private val formAnswerRepository: FormAnswerRepository` on `CollectionBatchListViewModel` now triggers an "init-only property" weak warning** because the repo is only consumed in the `_existingAnswersBySessionUnitId` field initializer. Left as-is for consistency with the rest of the file's repo-param block (all `private val`); audit-pass §1 can decide whether the codebase wants a "drop `val` for init-only deps" convention.
...
- ~~`existingAnswersBySessionUnitId` (in `CollectionBatchFormViewModel`) vs. `answersBySessionUnitId` (in `CollectionBatchListViewModel`) — different names for the same shape returned by the same repo call. Pick one.~~ ✅ **Resolved in PR 3d, slice 2** — both VMs now use `existingAnswersBySessionUnitId` after the list VM was flipped to the `observe*` Flow shape.
...
**Note (post-slice-2):** the suspend `get` variant has been deleted; only `observeSessionUnitScopedFormAnswersBySessionId` (Flow) remains on `FormAnswerDao` / `FormAnswerRepository`. The naming audit still applies — the `observe…` sibling inherits the same verbose name.
