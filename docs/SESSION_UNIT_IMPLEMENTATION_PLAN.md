...
### PR 3c — In progress (uncommitted on `feat/collection-cycle`)

**What's landed in the working tree so far.** A "view-only" slice of 3c — enough to render the `CollectionBatchForm` end-to-end and unblock the SESSION / SESSION_UNIT scope split on intake, but **no persistence**. Submit, identity utilities, and edit-mode hydration are still pending. Captured here so the next session knows exactly what state the branch is in.
...
3. **New `collection_batch/form/presentation/` package**:
   - `CollectionBatchFormState` — `isLoading`, `sessionId`, `sessionUnitId`, `form`, `formQuestions`, `formAnswersByQuestionId: Map<Int, FormAnswer>` (full `FormAnswer` to keep the persistence step a small delta from current shape, matching `IntakeState`).
   - `CollectionBatchFormAction` — `ReturnToCollectionBatchListScreen`, `UpdateFormAnswer(questionId, value)`.
   - `CollectionBatchFormEvent` — `NavigateBackToCollectionBatchListScreen`.
   - `CollectionBatchFormViewModel` — `CoreViewModel` subclass; reads route via `savedStateHandle.toRoute<Destination.CollectionBatchForm>()`; fetches SESSION_UNIT questions on init; seeds a blank `FormAnswer` per question (random `localId`, `value = ""` or `"false"` for booleans, `submittedAt = 0L`); `UpdateFormAnswer` mutates only `_state` (in-memory) with the same prerequisite-driven blanking-of-dependents logic copied from `IntakeViewModel`.
   - `CollectionBatchFormScreen` — partitions `state.formQuestions` by `isUnitIdentityComponent` inline (not in state, not memoized), renders an "identity" tile first then a "Batch Information" tile, both populated via `DynamicFormField` with prerequisite gating.
   - `components/CollectionBatchFormTile` — local tile component. **Does not reuse `IntakeTile`.** Built on `InfoTile` from `core.presentation.components.tile` with the same Row-with-divider-icon-title shape.
...
5. **`DefaultIntakeFieldsCache.formAnswers` removed end-to-end.** Five files touched: `DefaultIntakeFieldsCacheDto.kt` (field dropped), `DefaultIntakeFieldsCache.kt` interface (param dropped), `DefaultIntakeFieldsCacheImplementation.kt` (param + DTO assignment dropped), `IntakeViewModel.kt` (the `answersToCache` build block + `formAnswers =` arg + the load-time `cachedFormAnswers` read + the `cachedFormAnswers[question.id] ?: ...` fallback in the seed all removed), `SettingsViewModel.kt` (the `formAnswers = emptyMap()` arg on data-sync write removed). DataStore migration is a no-op — `DefaultIntakeFieldsCacheDtoSerializer` already sets `ignoreUnknownKeys = true`, so existing on-device JSON containing `"formAnswers": {...}` deserializes cleanly and the field is silently dropped on next write.

**Deviations from plan worth recording.**

1. **`getQuestionsByFormId` was renamed to `getQuestionsByFormIdAndScope`, not just widened in place.** The plan's "Additional scope #2" (line 309-332) prescribes widening while keeping the original name. The actual change widens and renames. After audit this was accepted: it's still a single method with a single query body, no mapper duplication, no second contract — Rule 2's "no sibling" intent is preserved. The name choice is a preference (caller-site readability) not a Rule 2 violation. If a future caller pops up that wants "all questions", it calls `getQuestionsByFormIdAndScope(formId, null)` rather than the cleaner `getQuestionsByFormId(formId)`; this trade-off was deemed acceptable.
2. **`CollectionBatchFormState.formAnswersByQuestionId` is `Map<Int, FormAnswer>`, not `Map<Int, String>`.** The simpler `String` shape would be enough for rendering today, but the full-`FormAnswer` shape mirrors `IntakeState.formAnswersByQuestionId` so the persistence step in the next slice is a smaller delta (no per-question `FormAnswer` reconstruction at save time). Acceptable trade-off.
3. **No `BackHandler` on `CollectionBatchFormScreen`.** `IntakeScreen` and `AddHourScreen` both have one. Since the screen has no draft persistence, system back and the back-icon tap are semantically identical (both discard in-memory state and pop) — the `BackHandler` would be dead indirection. Add when/if `ReturnToCollectionBatchListScreen` grows side effects (confirm-discard dialog, save-as-draft, etc.).

**Deliberately deferred to the rest of 3c.**

- Persistence: `FormAnswerRepository.upsertFormAnswer` still doesn't take `sessionUnitId`; `SessionUnitRepository.upsertSessionUnit` doesn't exist yet; no `TransactionHelper`-wrapped submit. None of `FormAnswerRepository`, `SessionUnitRepository`, or `TransactionHelper` are injected into `CollectionBatchFormViewModel` yet.
- Submit terminal action + the confirmation dialog from "Additional scope #1".
- `CollectionBatchIdentityResolver` / `CollectionBatchIdentityValidator`.
- Cross-unit duplicate-identity validation.
- Edit-mode hydration: `loadFormDetails` ignores `sessionUnitId` — when non-null it should hydrate the seed map from existing `form_answer` rows for the unit (mirror `IntakeViewModel.loadFormDetails:679` with a unit-scoped read).
- Silent failure handling: `programId == null` / `program == null` / `form == null` currently just drop `isLoading` to `false` and leave an empty screen. Compare `IntakeViewModel:636-650` which emits an error + nav-back event. A `CollectionBatchFormError` enum + `NavigateBackToCollectionBatchListScreen` emission should land here.
- `CollectionBatchCard` title still resolves to `"Batch ${unit.unitOrder}"` — identity-resolver wiring on the list card is part of 3c.

**New conventions / best practices that emerged from this slice and should propagate.**

1. **Derived state belongs in the screen, not the VM or state.** If a value is a cheap, pure transform of state already on screen, derive it inline. Only promote to `remember` when something downstream needs identity stability (e.g. `LazyColumn` `items` keys, `LaunchedEffect` keys, `derivedStateOf` sources). Only promote to state when a sibling reads it or persistence depends on it. Precedent already in the codebase: `IntakeScreen`'s in-line `answerMap` (line ~530), `isOtherCollectionMethod`, `isOtherSpecimenCondition`. Concrete cleanup candidates in `IntakeState` that this rule identified for a future pass:
   - **`isCurrentCollectorMissing`** → move to the screen as a derived `val` over `session.collectorName/title` × `allCollectors`. Eliminates the load-time block at `IntakeViewModel:732-737` and 3 `_state.update { isCurrentCollectorMissing = ... }` reassignments (`SelectCollector`, `RegisterMissingCollector.onSuccess`, load).
   - **`isCollectionMethodTooltipVisible`** → `rememberSaveable` in the screen. Pure UI ephemeral, no side effects, no persistence. Also drops `IntakeAction.Show/HideCollectionMethodTooltipDialog` and their two VM branches.
   - **Adjacent bug surfaced while auditing**: `_allCollectors` is observed twice in `IntakeViewModel` — the outer `combine` (lines 86-96) writes `allCollectors` into the exposed state, and `loadFormDetails`'s inner `combine` (~line 760) also writes it via a separate subscription. The outer always wins on emission; the inner write is dead and the inner subscription is redundant. Worth dropping when convenient.
2. **Feature boundary discipline.** Presentation components from one feature package should not be imported by another. Local tile (`CollectionBatchFormTile`) is preferred over reaching into `intake.presentation.components.IntakeTile`. Adjacent leaks still present in the 3c diff: `DynamicFormField` (`intake.presentation.components`) and `FormQuestionPrerequisiteEvaluator` (`intake.domain.util`). Both operate purely on core domain types (`FormQuestion`, `FormQuestionPrerequisiteExpression`) with no intake-specific dependency. The clean fix is to promote them to `core.presentation.components.form/` and `core.domain.util/` respectively (where `DropdownField`, `TextEntryField`, `ToggleField`, `DatePickerField` already live). Not in 3c's scope but a clear follow-up refactor.
3. **`DefaultIntakeFieldsCache` is for stable user/device/site context only.** Volatile per-session content (form answers, in-progress draft state) belongs in `FormAnswerRepository` (in-session hydration via `getFormAnswersBySessionId`) and is intentionally wiped between sessions. Once `answerScope` split SESSION / SESSION_UNIT, the cache's flat `Map<Int, String>` had no scope discriminator, so cross-session restore could silently apply SESSION_UNIT defaults to a SESSION form (or vice versa). Removed end-to-end. Cross-session memory in the cache is now strictly: collector identity, last-trained-on, hardware ID, district, village, locationSelections.
4. **`@Serializable(ignoreUnknownKeys = true)` is the migration story for DataStore DTO field removal.** No version bump, no migration code. Already configured on `DefaultIntakeFieldsCacheDtoSerializer`. Future field removals on cache DTOs can follow the same pattern.
5. **Rule 2 nuance — "no sibling" ≠ "no rename".** Widening an existing function with a new parameter and renaming it is still Rule 2 compliant: the anti-pattern is *adding a second method* with overlapping responsibility (`getQuestionsByFormId(formId)` + `getQuestionsByFormIdAndScope(formId, scope)` coexisting as parallel reads). Replacing one with the other — same query body, same return shape, just a wider signature and clearer name — is fine. The audit-time disagreement on this point resolved as: name is a preference, single-method-body is the actual rule.
